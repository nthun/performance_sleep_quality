...
# install.packages(c("tidyverse", "janitor","psych","lmerTest", "robustlmm","cowplot", "MASS", "DT", "MASS"))
...
df_raw <- read_csv2("data/PSQI_data_SES_MEQ.csv", 
...
# Exploraton of association of possible domgraphic confounders
```{r}
df_raw %>% 
  transmute(
            sex = nem, 
            age = `életkor`, 
            education_yrs = `iskolázottság`,
            family_income = `Család gazdasági helyzete`, 
            own_income = `Saját gazdasági helyzet`,
            mother_education = `Anya iskolai végzettsége években`,
            father_education = `Apa iskolai végzettsége években`,
            house_rooms = as.numeric(`Hány szobás a ház`),
            house_area = as.numeric(`Mekkora a ház alapterülete`),
            number_of_cars = `Hány autója van a családnak`) %>% 
...
all_models <- 
  df %>% 
  select(one_of(variables_needed), -one_of(sleep_variables), one_of(confounders)) %>% 
  gather(outcome_var, value, -sleep_disturb, -study, -one_of(confounders)) %>% 
  drop_na(sleep_disturb, value) %>% 
  split(.$outcome_var) %>% 
  map(., ~rlmer(scale(value) ~ scale(sleep_disturb) + scale(meq_all) + sex + age + (1|study), 
                REML = FALSE,
                data = .x))
...
# Get the degrees of freedom (number of observations)
dfs <- 
  map_int(all_models, ~length(.x@resp$y)) %>% 
  tibble(outcome_var = names(.),
         df =  .)
...
models %>% 
  ...
  select(group, outcome_var, everything()) %>% 
    ...
    options = list(dom = 'Bfrtip', 
                   buttons = c('excel', "csv"))) %>% 
  formatRound(columns=c("estimate", "std.error", "statistic","p"), digits = 4)
```
...
null_BIC <-
  ...
  gather(outcome_var, value, -sleep_disturb, -study, -one_of(confounders)) %>% 
  split(.$outcome_var) %>%
  map_df(., ~lmer(scale(value) ~ 1 + scale(meq_all) + sex + age + (1|study), 
                  REML = FALSE, 
                  data = .x) %>% 
             glance(.) %>% 
...
df %>% 
  ...
  gather(outcome_var, value, -sleep_disturb, -study, -one_of(confounders)) %>% 
  split(.$outcome_var) %>% 
  map(., ~lmer(scale(value) ~ scale(sleep_disturb) + scale(meq_all) + sex + age + (1|study), REML = FALSE,
               data = .x)) %>% 
  map_df(~broom::glance(.),
         .id = "outcome_var") %>%
