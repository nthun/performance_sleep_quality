...
df_raw %>% 
  select(
            ...
            house_rooms = `Hány szobás a ház`,
            house_area = `Mekkora a ház alapterülete`,
            number_of_cars = `Hány autója van a családnak`) %>% 
  mutate(house_rooms = as.numeric(house_rooms),
         house_area = as.numeric(house_area)) %>% 
  bind_cols(select(df, one_of(variables_needed), -study)) %>% 
