...
all_models <- 
  ...
  split(.$outcome_var) %>% 
  map(., ~rlmer(scale(value) ~ scale(sleep_disturb) + scale(meq_all) + sex + (1|study), 
                REML = FALSE,
                data = .x))
...
null_BIC <-
  ...
  gather(outcome_var, value, -sleep_disturb, -study, -one_of(confounders)) %>% 
  split(.$outcome_var) %>%
  map_df(., ~lmer(scale(value) ~ 1 + scale(meq_all) + sex + (1|study), 
                  REML = FALSE, 
                  data = .x) %>% 
             glance(.) %>% 
...
df %>% 
  ...
  gather(outcome_var, value, -sleep_disturb, -study, -one_of(confounders)) %>% 
  split(.$outcome_var) %>% 
  map(., ~lmer(scale(value) ~ scale(sleep_disturb) + scale(meq_all) + sex + (1|study), REML = FALSE,
               data = .x)) %>% 
  map_df(~broom::glance(.),
         .id = "outcome_var") %>%
...
study2_nullBIC <-
  study2_long %>% 
  mutate(model = map(data, ~MASS::rlm(scale(out_value) ~ 1 + scale(meq_all) + sex,
                                data = .x) %>% 
                           glance(.) %>% 
                           select(nullBIC = BIC))) %>% 
  unnest(model) %>% 
...
study2_models <-
  study2_long %>% 
  mutate(model = map(data, ~MASS::rlm(scale(out_value) ~ scale(pred_value) + scale(meq_all) + sex,
                                data = .x)),
